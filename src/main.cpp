...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (pindexLast->nTime > 1485302400)	// Jan 25 2017, 00:00:00 GMT
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
		...
		int64_t nMaxOffset = 7 * 86400; // Seven days
		
		// Don't accept blocks with future timestamps
		if (pindexPrev->nHeight > 200000 && nMedianTimePast  + nMaxOffset < GetBlockTime())
